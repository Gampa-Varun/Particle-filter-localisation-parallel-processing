...
int main() {
	...
	for(int lambda =0 ; lambda<position_meas.size();lambda++){
		std::ofstream dataFile;
		dataFile.open("true_particle.dat", std::ios::app);
		dataFile << gt[lambda].x << " " << gt[lambda].y << " " << gt[lambda].theta << "\n";
		dataFile.close();
	}
	...
	for (int i = 0; i < num_time_steps; ++i) {
		...
		//cout<< "erx"<<	best_particle.x-gt[i].x<<endl;
	}
